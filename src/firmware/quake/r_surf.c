...
static void R_DrawSurfaceBlock8_unified (void);

static void	(*surfmiptable[4])(void) = {
	R_DrawSurfaceBlock8_unified,
	R_DrawSurfaceBlock8_unified,
	R_DrawSurfaceBlock8_unified,
	R_DrawSurfaceBlock8_unified
};
...
Unified surface block function for all mip levels.
Hardware handles all rows autonomously with bilinear light interpolation.
================
*/
static void R_DrawSurfaceBlock8_unified (void)
{
	...
	for (int v = 0; v < r_numvblocks; v++)
	{
		...
		span_draw_surface_block(
			...
			r_lightptr[r_lightwidth], r_lightptr[r_lightwidth + 1],
			(unsigned int)sourcetstep, (unsigned int)surfrowbytes,
			blockdivshift);
...
================
*/
PQ_FASTTEXT void R_DrawSurfaceBlock8_mip0 (void)
{
	...
	for (v=0 ; v<r_numvblocks ; v++)
	{
		...
		int lr = r_lightptr[1];
		r_lightptr += r_lightwidth;
		int lls = (r_lightptr[0] - ll) >> 4;
		int lrs = (r_lightptr[1] - lr) >> 4;
	...
	for (v=0 ; v<r_numvblocks ; v++)
	{
		...
		for (i=0 ; i<16 ; i++)
		{
			...
			for (b=15; b>=0; b--)
			{
				pix = psource[b];
#if HW_CMAP_BRAM
				prowdest[b] = CMAP_BRAM_PTR[(light & 0xFF00) + pix];
#else
...
================
*/
PQ_FASTTEXT void R_DrawSurfaceBlock8_mip1 (void)
{
	int				v, i, b, lightstep, lighttemp, light;
	unsigned char	pix, *psource, *prowdest;
	...
	for (v=0 ; v<r_numvblocks ; v++)
	{
		...
		int lr = r_lightptr[1];
		r_lightptr += r_lightwidth;
		int lls = (r_lightptr[0] - ll) >> 3;
		int lrs = (r_lightptr[1] - lr) >> 3;
...
================
*/
PQ_FASTTEXT void R_DrawSurfaceBlock8_mip2 (void)
{
	int				v, i, b, lightstep, lighttemp, light;
	unsigned char	pix, *psource, *prowdest;
	...
	for (v=0 ; v<r_numvblocks ; v++)
	{
		...
		int lr = r_lightptr[1];
		r_lightptr += r_lightwidth;
		int lls = (r_lightptr[0] - ll) >> 2;
		int lrs = (r_lightptr[1] - lr) >> 2;
...
================
*/
PQ_FASTTEXT void R_DrawSurfaceBlock8_mip3 (void)
{
	int				v, i, b, lightstep, lighttemp, light;
	unsigned char	pix, *psource, *prowdest;
	...
	for (v=0 ; v<r_numvblocks ; v++)
	{
		...
		int lr = r_lightptr[1];
		r_lightptr += r_lightwidth;
		int lls = (r_lightptr[0] - ll) >> 1;
		int lrs = (r_lightptr[1] - lr) >> 1;
