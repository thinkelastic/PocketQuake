...
static void NET_SetListening (qboolean state)
{
	listening = state;
...
static void NET_Listen_f (void)
{
	...
	NET_SetListening (Q_atoi(Cmd_Argv(1)) ? true : false);
}
...
static void MaxPlayers_f (void)
{
	...
	if ((n == 1) && listening)
		NET_SetListening (false);

	if ((n > 1) && (!listening))
		NET_SetListening (true);
...
qsocket_t *NET_CheckNewConnections (void)
{
	...
	for (net_driverlevel=0 ; net_driverlevel<net_numdrivers; net_driverlevel++)
	{
		if (net_drivers[net_driverlevel].initialized == false)
		{
			if (net_driverlevel == 1 && diag_count < 3)
			{
				Con_Printf("NET: drv1 not initialized\n");
				diag_count++;
			}
			continue;
		}
		if (net_driverlevel && listening == false)
		{
			if (net_driverlevel == 1 && diag_count < 3)
			{
				Con_Printf("NET: drv1 skip, listening=0\n");
				diag_count++;
			}
			continue;
		}
