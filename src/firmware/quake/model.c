...
static unsigned int Mod_NameTag(const char *s)
{
	unsigned int t = 0;
	if (!s) return 0;
	if (s[0]) t |= (unsigned int)(unsigned char)s[0];
	if (s[1]) t |= (unsigned int)(unsigned char)s[1] << 8;
	if (s[2]) t |= (unsigned int)(unsigned char)s[2] << 16;
	if (s[3]) t |= (unsigned int)(unsigned char)s[3] << 24;
	return t;
}
...
void CalcSurfaceExtents (msurface_t *s)
{
	...
	{	
		...
		/* Surface extents check removed - soft-float floor/ceil on RISC-V
		   produces different rounding than x86 FPU, causing false failures */
	}
}
