...
PQ_FASTDATA int		sintable[SIN_BUFFER_SIZE];
PQ_FASTDATA int		intsintable[SIN_BUFFER_SIZE];
...
================
*/
PQ_FASTTEXT void R_EmitEdge (mvertex_t *pv0, mvertex_t *pv1)
{
	edge_t	*edge, *pcheck;
	int		u_check;
	float	u, u_step;
...
================
*/
PQ_FASTTEXT void R_ClipEdge (mvertex_t *pv0, mvertex_t *pv1, clipplane_t *clip)
{
	float		d0, d1, f;
	mvertex_t	clipvert;
...
// FIXME: cache this?
	...
	// Bias bmodel surfaces slightly closer to camera so coplanar
	// bmodel/world surfaces don't z-fight with float32 precision.
	// Proportional to distinv so it works at all distances.
	{
		float bias = distinv * 0.002f;
		surface_p->d_ziorigin += (bias > 0) ? bias : -bias;
	}
