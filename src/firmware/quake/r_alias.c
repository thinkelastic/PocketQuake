...
#if HW_ALIAS_MAC
	atm_load_matrix(aliastransform);
#endif
...
================
*/
PQ_FASTTEXT void R_AliasTransformFinalVert (finalvert_t *fv, auxvert_t *av,
	trivertx_t *pverts, stvert_t *pstverts)
{
#if HW_ALIAS_MAC
	ATM_VERT_IN = pverts->v[0] | (pverts->v[1] << 8) |
	              (pverts->v[2] << 16) | (pverts->lightnormalindex << 24);

	av->fv[0] = (float)ATM_RESULT_VX * (1.0f / 65536.0f);
	av->fv[1] = (float)ATM_RESULT_VY * (1.0f / 65536.0f);
	av->fv[2] = (float)ATM_RESULT_VZ * (1.0f / 65536.0f);
...
================
*/
PQ_FASTTEXT void R_AliasTransformAndProjectFinalVerts (finalvert_t *fv, stvert_t *pstverts)
{
	int			i;
	trivertx_t	*pverts;
#if !HW_ALIAS_MAC
	...
	{
#if HW_ALIAS_MAC
	// Hardware MAC: write packed vertex, read unscaled viewspace results
		ATM_VERT_IN = pverts->v[0] | (pverts->v[1] << 8) |
		              (pverts->v[2] << 16) | (pverts->lightnormalindex << 24);

	// Read viewspace Z first (blocks until HW done), start fdiv
		{
			float vz = (float)ATM_RESULT_VZ * (1.0f / 65536.0f);
			zi = 1.0f / vz;
		}

	// zi equivalent to original 2^31/vz (for z-buffer)
		fv->v[5] = (int)(zi * ((float)0x8000 * (float)0x10000));

	// Project to screen using unscaled viewspace XY
		{
			float vx = (float)ATM_RESULT_VX * (1.0f / 65536.0f);
			float vy = (float)ATM_RESULT_VY * (1.0f / 65536.0f);
...
PQ_FASTTEXT void R_AliasDrawModel (alight_t *plighting)
{
	...
	if (!atm_normals_loaded) {
		atm_load_normals(r_avertexnormals, NUMVERTEXNORMALS);
		atm_normals_loaded = 1;
	}
#endif
