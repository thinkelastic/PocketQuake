...
void D_DrawNonSubdiv (void)
{
	...
	for (i=0 ; i<lnumtriangles ; i++, ptri++)
	{
		...
		__builtin_memcpy(r_p0, index0->v, 6 * sizeof(int));
		__builtin_memcpy(r_p1, index1->v, 6 * sizeof(int));
		__builtin_memcpy(r_p2, index2->v, 6 * sizeof(int));
...
static void FloorDivModInt(int numer, int denom, int *quotient, int *rem)
{
	int q, r;

	if (denom <= 0)
		Sys_Error ("FloorDivModInt: bad denominator %d\n", denom);
...
// TODO: implement x86 version
	...
	{
		FloorDivModInt (tm, tn, &ubasestep, &erroradjustup);
		erroradjustdown = tn;
	}
}
