...
#ifdef POCKET_QUAKE
	/* Low-rate liveness marker after full signon: one line every ~256 frames. */
	if (cls.signon == SIGNONS && ((host_framecount & 255) == 0) && live_hb < 32)
	{
		volatile unsigned int *dbg_key1 = (volatile unsigned int *)0x40000050;
		Sys_Printf ("LIVE f=%d st=0x%x k1=%08x t=%.2f\n",
		            host_framecount, pq_dbg_stage, *dbg_key1, cl.time);
		live_hb++;
	}
#endif
}
