...
void M_MultiPlayer_Key (int key)
{
	...
	case K_ENTER:
		...
		case 0:	// Join game
			if (serialAvailable || ipxAvailable || tcpipAvailable)
			{
				if (tcpipAvailable && !serialAvailable && !ipxAvailable)
				{
					m_net_cursor = 3;	// TCPIPConfig (link cable)
					M_Menu_Search_f ();
				}
				else
					M_Menu_Net_f ();
...
void M_Menu_GameOptions_f (void)
{
	...
	if (TCPIPConfig && !serialAvailable && !ipxAvailable)
		maxplayers = 2;	// link cable is point-to-point
}
